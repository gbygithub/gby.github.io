...
define(["OTMap/Utils/ColorUtil", "esri/Color", "OTMap/components/Legend", "esri/symbols/TextSymbol", "esri/geometry/Polygon", "esri/geometry/Point", "esri/geometry/Circle",
        ...
        "esri/renderers/smartMapping", "esri/renderers/ClassBreaksRenderer", "esri/renderers/HeatmapRenderer", "esri/renderers/SimpleRenderer","esri/symbols/PictureMarkerSymbol"],
    function (ColorUtil, Color, Legend, TextSymbol, Polygon, Point, Circle,
              ...
              smartMapping, ClassBreaksRenderer, HeatmapRenderer, SimpleRenderer, PictureMarkerSymbol) {
        function DrawUtil() {
        }
        ...
        DrawUtil.prototype.createSLayer = function (me, callback) {
            ...
            if (me.config.layer.url && me.config.layer.url != "") {
                GetFeatrueByQuery(me,callback, infoTemplate); //整个图层方式
            }
            else {
                GetLayerData(me.config.layer.data,callback, infoTemplate);//获取外部数据方式
            }
    
            return obj;
        };

        function GetFeatrueByQuery(me,callback, infoTemplate)
        {
            ...
            query.outFields = ["*"];
            query.where = "1=1";
            query.outSpatialReference = me.map.spatialReference;
            layer.queryFeatures(query, function (data) {
                GetLayerData(me,data,layerConfig,callback, infoTemplate);
            });
        }

        function GetLayerData(me,data,layerConfig,callback, infoTemplate)
        {
            data.fields.push({
                "name": diffField,
                "alias": "区分颜色",
        ...
        DrawUtil.prototype.drawPie = function (me) {
            ...
            me._features.forEach(function (feature) {
                ...
                var standRadius = (me._features[0]._extent.ymax - me._features[0]._extent.ymin) / 1.5;
        ...
        //简单渲染图层
        DrawUtil.prototype.SimpleRenderLayer = function (me) {
            var obj = this;
            var symbol;

            if (me.config.style.type == "point")//点类型
            {
                var jpg = me.config.style.url;
                symbol = new PictureMarkerSymbol(jpg, 51, 51);//点符号图片
            }
            else{
                ...
                new Color([me.config.style.outlinecolor]),//边线颜色及透明度
                1
                ),
                new Color([me.config.style.color])//[125, 125, 125, 0.35] 填充颜色及透明度
                );
            }
            var SimpleRenderer = new SimpleRenderer(symbol);

            me.drawLayer.setRenderer(SimpleRenderer);
            return obj;
        };
        /* 辅助方法 */
